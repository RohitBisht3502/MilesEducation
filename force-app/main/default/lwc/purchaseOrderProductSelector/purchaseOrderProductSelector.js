...
hasShippingAddress = false;
    ...
    if (this.hasBillingAddress) {
        options.push({ label: 'Billing Address', value: 'billing' });
    }

    if (this.hasShippingAddress) {
        options.push({ label: 'Shipping Address', value: 'shipping' });
    }
    ...
    return this.products.filter(p =>
        ...
        handleDiscountValue(event) {
            ...
            if (this.discountType === 'fixed') {
                value = Math.min(value, this.subTotal);
            }
...
async proceedToCheckout() {
    ...
    try {
        ...
        // 🔹 Detect Study Material in cart
        const hasStudyMaterial = this.selectedProducts.some(
            p => p.type === 'Study Material'
        );
        ...
        // 🔴 Shipping required only for Study Material
